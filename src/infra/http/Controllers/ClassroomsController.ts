...
import { ClassroomUsecase } from "../../../application/usercase/Course/ClassroomUsecase";
import { HttpServer } from "../HttpServer";
import { Middleware } from "../Middleware";
import { Token } from "../../../domain/Token";
import { schemaValidationCourseCreate } from "../SchemaValidation";
import { validationResult } from "express-validator";

export class ClassroomsController {
  private readonly url = "/classrooms"
  constructor(private readonly http: HttpServer, private readonly usecase: ClassroomUsecase) {
    this.setupRoutes();
  }
  private setupRoutes() {
    this.http.on("get", `${this.url}`, [Middleware.isValidToUser], async (req: Request) => {
      return [await this.usecase.show(req.params), 200]
    })
    this.http.on("post", `${this.url}`, [Middleware.isValidToUser, ...schemaValidationCourseCreate], async (req: Request) => {
